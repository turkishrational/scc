...
#ifdef _INASM
  #asm
  xor eax,eax	  ; set search value to zero
  mov ecx,65535	  ; set huge maximum
  mov edi,[ebp+8] ; get address of s
  cld		  ; set direction flag forward
  repne scasb 	  ; scan for zero
  mov eax,65534
  sub eax,ecx	  ; calc and return length
  #endasm
#endif
}
...
{int backup; char *next;
 ...
 next=str;
 while(--size>0)
 {switch (*next=OS_fgetc(fd)) /* *next=fgetc(fd) */
  {case EOF:
    *next=NULL;
